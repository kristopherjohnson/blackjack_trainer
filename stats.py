class Statistics:
    ...
    def get_category_accuracy(self, category):
        data = self.session_data['by_category'].get(category, {'correct': 0, 'total': 0})
        if data['total'] == 0:
            return 0.0
        return (data['correct'] / data['total']) * 100
    
    def get_dealer_strength_accuracy(self, strength):
        data = self.session_data['by_dealer_strength'].get(strength, {'correct': 0, 'total': 0})
        if data['total'] == 0:
            return 0.0
        return (data['correct'] / data['total']) * 100
    ...
    def get_session_accuracy(self):
        if self.session_data['total_attempts'] == 0:
            return 0.0
        return (self.session_data['correct_answers'] / self.session_data['total_attempts']) * 100
    ...
    def display_progress(self):
        ...
        print(f"Overall: {correct}/{total} ({self.get_session_accuracy():.1f}%)")
        ...
        for hand_type in ['hard', 'soft', 'pair']:
            ...
            if data['total'] > 0:
                ...
                print(f"  {hand_type.capitalize()}: {data['correct']}/{data['total']} ({accuracy:.1f}%)")
        ...
        for strength in ['weak', 'medium', 'strong']:
            ...
            if data['total'] > 0:
                accuracy = (data['correct'] / data['total']) * 100
                print(f"  {strength.capitalize()}: {data['correct']}/{data['total']} ({accuracy:.1f}%)")
