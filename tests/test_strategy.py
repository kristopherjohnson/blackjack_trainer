...
class TestStrategyChart(unittest.TestCase):
    ...
    def test_hard_11_strategy(self):
        ...
        for dealer in range(2, 12):
            ...
            if dealer <= 10:
                ...
                self.assertEqual(action, 'H', 
                    f"Hard 11 vs Ace should be Hit")
    ...
    def test_soft_13_14_strategy(self):
        ...
        for total in [13, 14]:
            for dealer in range(2, 12):
                ...
                if 5 <= dealer <= 6:
                    self.assertEqual(action, 'D', 
                        f"Soft {total} vs {dealer} should be Double")
                else:
                    self.assertEqual(action, 'H', 
    ...
    def test_pairs_2_3_strategy(self):
        ...
        for pair_val in [2, 3]:
            for dealer in range(2, 12):
                action = self.chart.get_correct_action('pair', pair_val, dealer)
                if 2 <= dealer <= 7:
                    self.assertEqual(action, 'Y', 
                        f"Pair of {pair_val}s vs {dealer} should be Split")
                else:
                    self.assertEqual(action, 'H', 
                        f"Pair of {pair_val}s vs {dealer} should be Hit")
    ...
    def test_absolute_rules(self):
        ...
        self.assertTrue(self.chart.is_absolute_rule('hard', 17, 10)) # Hard 17+
...
class TestStrategyChartComprehensive(unittest.TestCase):
    ...
    def test_all_hard_totals_coverage(self):
        ...
        for total in range(5, 22):
            for dealer in range(2, 12):
                action = self.chart.get_correct_action('hard', total, dealer)
                self.assertIn(action, ['H', 'S', 'D'], 
                    f"Invalid action '{action}' for Hard {total} vs {dealer}")
    ...
    def test_all_soft_totals_coverage(self):
        ...
        for total in range(13, 22):
            for dealer in range(2, 12):
                action = self.chart.get_correct_action('soft', total, dealer)
                self.assertIn(action, ['H', 'S', 'D'], 
                    f"Invalid action '{action}' for Soft {total} vs {dealer}")
